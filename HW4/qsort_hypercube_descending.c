...
int * merged_list(int * list1, int list1_size, int * list2, int list2_size) {
	...
	if (list1[idx1] > list2[idx2]) {                    //*** ALTERED: to switch merged list orderinge
	    list[idx] = list1[idx1]; 
	    idx++; idx1++;
	} else {
	    list[idx] = list2[idx2]; 
...
int split_list_index (int *list, int list_size, int pivot) {
    ...
    while (first < last) {
        if (list[mid] >= pivot) {                       //*** ALTERED: to switch max for min
...
int compare_int(const void *a0, const void *b0) {
    int a = *(int *)a0;
    int b = *(int *)b0;
    if (a > b) {                                        //*** ALTERED: to reverse comparison order
	return -1; 
    } else if (a < b) {                                 //*** ALTERED: to reverse comparison order
	return 1;
    } else {
	return 0;
    }
